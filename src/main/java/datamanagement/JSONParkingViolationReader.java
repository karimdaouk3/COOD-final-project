...
public class JSONParkingViolationReader implements ParkingViolationReader {
    ...
    public JSONParkingViolationReader(String filePath) {
        if (filePath == null || filePath.trim().isEmpty()) {
            throw new IllegalArgumentException("File path cannot be null or empty");
        }
        this.filePath = filePath;
        this.parsingStrategy = new JSONParsingStrategy();
    }

    public JSONParkingViolationReader(String filePath, FileParsingStrategy parsingStrategy) {
        if (filePath == null || filePath.trim().isEmpty()) {
            throw new IllegalArgumentException("File path cannot be null or empty");
        }
    ...
    public List<ParkingViolation> readParkingViolations() throws IOException {
        ...
        for (String[] parts : records) {
            if (parts.length < 7) {
                throw new IllegalArgumentException("Invalid JSON record format: expected 7 fields");
            }
